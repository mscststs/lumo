...
export const zh: TranslationSchema = {
  ...
  options: {
    ...
    models: {
      ...
      fetchModels: '自动获取',
      fetchModelsConfigError: '请先配置接口地址和 API 密钥。',
      fetchModelsError: '模型列表获取失败。',
      modelName: '显示名称',
    ...
    commands: {
      title: '命令',
      description: '以 / 开头的快捷输入。',
      enableCommands: '启用命令',
      enableCommandsDesc: '关闭后，在输入框中输入 / 不会触发任何命令，也不会展开短语。',
      applyTiming: '触发时机',
      applyTimingSend: '发送时',
      applyTimingSelect: '选中时',
      builtinSection: '系统',
      userSection: '自定义',
      addCommand: '添加命令',
      editCommand: '编辑命令',
      name: '触发词',
      nameHint: '字母、数字、连字符和下划线。输入时跟在 / 后面。',
      namePlaceholder: 'fy',
      phrase: '展开为',
      phraseHint: '发送时替换触发词；其后输入的文字会保留。',
      phrasePlaceholder: '翻译此页面',
      emptyUser: '还没有自定义命令。',
      enabled: '启用',
      deleteConfirm: '删除 /{{name}}？',
      conflictHint: '启用后将关闭：{{names}}',
      errors: {
        nameRequired: '请输入触发词。',
        nameWhitespace: '触发词中不能包含空格。',
        nameSlash: '不要包含 /，它会自动加上。',
    ...
    ui: {
      ...
      themeSystem: '跟随系统',
      fontSize: '字体大小',
      fontSizeVerySmall: '极小 (12px)',
      fontSizeSmall: '小 (13px)',
      fontSizeMedium: '中 (14px)',
      fontSizeMediumLarge: '中大 (15px)',
      fontSizeDefault: '默认 (16px)',
      fontSizeLarge: '大 (17px)',
      fontSizeVeryLarge: '极大 (18px)',
      groupGeneral: '全局',
      groupSidebar: '侧边栏与聊天',
      ...
      toolbarDelete: '删除',
      toolbarUsage: 'Token 用量',
      visibilityAll: '所有消息',
      visibilityAssistant: '仅 AI 消息',
      visibilityUser: '仅用户消息',
      visibilityHidden: '隐藏',
      visibilityShow: '显示',
    },
    chatDebug: {
    ...
    ocr: {
      title: '图片识别（OCR）',
      description: '当使用不支持视觉的模型时，自动调用视觉模型将图片转换为文本描述。',
      enabled: '启用 OCR',
      enabledDesc: '通过视觉模型将图片转换为文本，供非视觉模型使用。',
      provider: 'OCR 提供商',
      model: 'OCR 模型',
      modelHint: '仅显示支持视觉的模型。',
      noVisionModels: '未配置视觉模型。请先在“模型”设置中添加一个。',
      prompt: 'OCR 提示词',
      promptDesc: '随图片一起发送给视觉模型的指令。',
      resetPrompt: '恢复默认',
    },
  },
};
